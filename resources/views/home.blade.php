...
<section class="hero-pattern text-white">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-24">
        <div class="grid lg:grid-cols-[300px_1fr] gap-12 lg:gap-16 items-start">
            {{-- Portrait --}}
                ...
                <div class="hero-portrait-frame">
                    ...
                    @else
                        <div class="w-full aspect-[4/5] bg-white/10 flex items-center justify-center">
                            <span class="font-serif text-7xl font-bold text-white/40">{{ substr($profile->name, 0, 1) }}</span>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="mt-5 space-y-1.5 text-center lg:text-left">
                        @if($profile->email)
                            <p class="text-sm text-white/75">
                                <a href="mailto:{{ $profile->email }}" class="inline-flex items-center gap-2 hover:text-white transition-colors">
                                    <svg class="w-3.5 h-3.5 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                    {{ $profile->email }}
                                </a>
                            </p>
                        @endif
            ...
            <div class="text-center lg:text-left">
                ...
                @if($profile->title)
                    <p class="mt-5 text-lg sm:text-xl text-white/92 font-medium leading-snug max-w-2xl">{{ $profile->title }}</p>
                @endif
                @if($profile->affiliation)
                    <p class="mt-4 text-base text-white/78 leading-relaxed max-w-2xl">{{ $profile->affiliation }}</p>
                @endif
                @if($profile->secondary_affiliation)
                    <p class="mt-2.5 text-sm text-white/55 italic max-w-2xl">{{ $profile->secondary_affiliation }}</p>
                @endif
                @if($profile->tagline)
                    <blockquote class="mt-8 text-sm sm:text-base text-white/65 leading-relaxed border-l-[3px] border-[var(--secondary)] pl-5 max-w-xl mx-auto lg:mx-0">
                        {{ $profile->tagline }}
                    </blockquote>
                @endif
                ...
                @if($stats)
                    <div class="mt-6 hero-stat-panel">
                        <div class="grid grid-cols-2 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-white/10">
                            <div class="px-4 py-4 text-center">
                                ...
                                <div class="mt-1 text-[0.625rem] font-semibold uppercase tracking-[0.18em] text-white/50">Publications</div>
                            </div>
                            <div class="px-4 py-4 text-center">
                                <div class="font-serif text-2xl sm:text-3xl font-bold tabular-nums">{{ number_format($stats->total_citations) }}+</div>
                                <div class="mt-1 text-[0.625rem] font-semibold uppercase tracking-[0.18em] text-white/50">Citations</div>
                            </div>
                            <div class="px-4 py-4 text-center">
                                <div class="font-serif text-2xl sm:text-3xl font-bold tabular-nums">{{ $stats->h_index }}</div>
                                <div class="mt-1 text-[0.625rem] font-semibold uppercase tracking-[0.18em] text-white/50">h-index</div>
                            </div>
                            <div class="px-4 py-4 text-center">
                                <div class="font-serif text-2xl sm:text-3xl font-bold tabular-nums">{{ $stats->i10_index }}</div>
                                <div class="mt-1 text-[0.625rem] font-semibold uppercase tracking-[0.18em] text-white/50">i10-index</div>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if(($workedWithOrganizations ?? collect())->isNotEmpty() || $socialLinks->isNotEmpty() || $profile->whatsappUrl())
                    <div class="mt-5 pt-5 border-t border-white/15 grid sm:grid-cols-[auto_minmax(0,1fr)] gap-5 sm:gap-6 items-start">
